...
namespace WebApi.Controllers
{
    ...
    {
        private readonly ContactExternalService _contactService;
        private readonly IMemoryCache _cache;

        public ContactController(ContactExternalService contactService, IMemoryCache cache)
        {
        ...
        public async Task<ActionResult<List<ContactResult>>> GetAllByDddAsync(int id)
        {
            try
            {
                ...
                return Ok(new ResultViewModel<IList<ContactResult>>(contacts));
            }
            catch (ArgumentException)
            {
                return BadRequest(new ResultViewModel<ContactResult>("01X03 - Invalid direct distance dialing code"));
            }
            catch (Exception)
            {
                return StatusCode(500, new ResultViewModel<ContactResult>("01X04- Internal server error"));
            }
        }
